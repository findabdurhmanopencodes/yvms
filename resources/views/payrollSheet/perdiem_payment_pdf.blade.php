...
<body>
    ...
    <h1 style="margin-top:10px; color:#01afee;text-align:center; font-size:25px;">
        <span style="text-align:center;">
            ...
            <p style="font-size: 12px"><b>   Trainee Perdiem payment Sheet</b></p>
    ...
    <hr>
     ...
     <table width="100%" style="font-size: 15px" class="table table-striped">
        <thead>
            ...
            <th style="text-align:left;"> Rate/daye </th>
        ...
        <tbody>
              @foreach ($placedVolunteers as $key => $placedVolunteer)
                <tr>
                     ...
                     <td>  {{ $sdate ->format("d/m/Y") }} - {{  Carbon\Carbon::parse($sdate)->addDays($day)->format('d-m-Y') }}
                    </td>
                     <td>   {{ number_format($scale [$key],2)}} </td>

                     <td>   {{ $day }} </td>
                     <td style="text-align:right;"> {{ number_format($day*$scale [$key],2)}}</td>
                     <td>  &nbsp;  &nbsp; &nbsp; &nbsp;   - </td>
                </tr>
