@extends('layouts.app')
@section('title', 'Woreda Intake')

@section('breadcrumbTitle', 'Woreda Intake Detail')
@section('breadcrumbList')
    <li class="breadcrumb-item">
        <a href="" class="text-muted">Woreda Intakes</a>
    </li>
    <li class="breadcrumb-item">
        <a href="" class="text-muted">{{ $woreda->name }} </a>
    </li>
@endsection
@push('css')
    <style>
...
@section('content')
    ...
    <div class="modal fade" id="addCapacity" tabindex="-1" role="dialog" aria-labelledby="addCapacityLabel"
        ...
        <div class="modal-dialog modal-lg" role="document">
            <form method="POST" action="{{ route('session.woreda.intake_store', ['woreda_id'=>$woreda->id, 'training_session'=>$trainingSession->id]) }}">
                {{-- <input type="hidden" name="trainingCenterId" value="{{ $region->id }}"> --}}
                @csrf
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="addCapacityLabel">Add Capacity To {{ $woreda->name }}</h5>
