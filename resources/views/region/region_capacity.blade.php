@extends('layouts.app')
@section('title', 'Region Intake')

@section('breadcrumbTitle', 'Rgion Intake Detail')
@section('breadcrumbList')
    <li class="breadcrumb-item">
        <a href="" class="text-muted">Region Intakes</a>
    </li>
    <li class="breadcrumb-item">
        <a href="" class="text-muted">{{ $region->name }} </a>
    </li>
@endsection
@push('css')
    <style>
...
@section('content')
    ...
    <div class="modal fade" id="addCapacity" tabindex="-1" role="dialog" aria-labelledby="addCapacityLabel"
        ...
        <div class="modal-dialog modal-lg" role="document">
            <form method="POST" action="{{ route('session.intake.store', ['region_id'=>$region->id, 'training_session'=>$trainingSession->id]) }}">
                {{-- <input type="hidden" name="trainingCenterId" value="{{ $region->id }}"> --}}
                @csrf
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="addCapacityLabel">Add Capacity To {{ $region->name }}</h5>
