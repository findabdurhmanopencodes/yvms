...
class ImportExportController extends Controller
{
    ...
    {
        $users = DB::table('volunteers')->join('woredas as w', 'volunteers.woreda_id', '=', 'w.id')->join('zones as z', 'w.zone_id', '=', 'z.id')->join('regions as r', 'z.region_id', '=', 'r.id')->leftJoin('statuses', 'volunteers.id', '=', 'statuses.volunteer_id')->where('acceptance_status','>=',Constants::VOLUNTEER_STATUS_PLACED)->leftJoin('approved_applicants', 'volunteers.id', '=', 'approved_applicants.volunteer_id')->leftJoin('training_placements', 'approved_applicants.id', '=', 'training_placements.approved_applicant_id')->leftJoin('training_center_capacities', 'training_placements.training_center_capacity_id', '=', 'training_center_capacities.id')->where('training_center_capacities.trainining_center_id', $id)->select('id_number', 'first_name','father_name','grand_father_name', 'phone', 'gender', 'r.name as regionname', 'z.name as zonename', 'w.name as woredaname')->get();
        // dd($users);
        return Excel::download(new CenterVolunteer($users, ['ID Number', 'First Name','Middle Name','Last Name', 'phone number', 'gender', 'Region', 'Zone', 'Woreda']), ' '.TraininingCenter::find($id)->code.'_placed_volunteers_list.xlsx');
    }
}
